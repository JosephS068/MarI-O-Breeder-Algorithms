...
-- Creation of random neuron, likely for mutation purposes
...
function mutate(genome)
	-- changes the various mutations rates
	genome.mutationRates["connections"] = MUTATION_RATE/2
	genome.mutationRates["link"] = 4 * MUTATION_RATE
	genome.mutationRates["bias"] = MUTATION_RATE
	genome.mutationRates["node"] = MUTATION_RATE
	genome.mutationRates["enable"] = MUTATION_RATE / 2
	genome.mutationRates["disable"] = MUTATION_RATE
...
function cullSpecies(cutToOne)
	for s = 1,#pool.species do
		...
		-- I believe the removal process takes place here
...
-- Cross over and creating next generation takes place here
function breedChildOriginal(species)
	local child = {}
	local max_fitness = species.topFitness

	local g1 = species.genomes[math.random(1, #species.genomes)]
	local g2 = species.genomes[math.random(1, #species.genomes)]

	local g1_survival = (max_fitness - g1.fitness) / max_fitness
	local g2_survival = (max_fitness - g2.fitness) / max_fitness

	while g1_survival < math.random(0, 1) and g2_survival < math.random(0, 1) do
		g1 = species.genomes[math.random(1, #species.genomes)]
		g2 = species.genomes[math.random(1, #species.genomes)]
	end

	average_fitness = calculateFormulaFitness(species)

	calculateCrossOver(max_fitness, species.averageFitness, g1, g2)

	if math.random() < CROSS_OVER_CHANCE then
	...
	calculateMutationRate(max_fitness, species.averageFitness, g1, g2)
...
function calculateCrossOver(maxFitness, averageFitness, parent1, parent2)
	larger_parent = parent1

	if parent1.fitness < parent2.fitness then
		larger_parent = parent2
	end
	if larger_parent.fitness < averageFitness then
		CROSS_OVER_CHANCE = K3
	else
		calculation = maxFitness - larger_parent.fitness
		if (maxFitness - averageFitness) == 0 then 
			CROSS_OVER_CHANCE = 0
		else
			calculation = calculation / (maxFitness - averageFitness)
			calculation = calculation * K1
			CROSS_OVER_CHANCE = calculation
		end
	end
end

function calculateMutationRate(maxFitness, averageFitness, parent1, parent2)
	larger_parent = parent1
	...
	if larger_parent.fitness < averageFitness then
		...
		if (maxFitness - averageFitness) == 0 then 
			...
			calculation = calculation / (maxFitness - averageFitness)
			calculation = calculation * K2
			MUTATION_RATE = calculation

			-- Due to the structure of this problem, we default to 0.1 rather than 0
...
function calculateFormulaFitness(species)
	total = 0
	for s =1, #species.genomes do
		total = total + species.genomes[s].fitness
	end
	total = total / #species.genomes
	return total
end
...
function newGenerationOriginal()
	-- have each species breed with one another to fill population/#species
	rankGlobally()
	removeStaleSpecies()
	rankGlobally()
	for s = 1,#pool.species do
	...
	for s = 1,#pool.species do
		...
		for i=1,breed do
			table.insert(children, breedChildOriginal(species))
		end
	end
...
function initializeRun()
	-- Loads the state selected by user
	savestate.load(config.NeatConfig.Filename);
	-- Checks if the user config if popup is selected
...
function writeTimeLapse(time)
	-- Opens a file in append mode
	file = io.open("adaptive.txt", "a")
	file:write("Generation " .. pool.generation .. " : " .. "Time: " .. tostring(time) .. " Fit: " .. pool.maxFitness .. "\n")
	file:close()
end
