...
function newGeneration()
	...
	elite = config.NeatConfig.Population * config.NeatConfig.ElitePopulation / 100
	for s = 1,#pool.species do
		local species = pool.species[s]
		-- sort the species by fitness
		table.sort(species.genomes, function (a,b)
		...
		breed = config.NeatConfig.Population
		for i=1,breed do
			if i < elite then table.insert(children, species.genomes[i]) 
			else table.insert(children, breedChild(species))
			end
		end
	end
...
function newGenerationOriginal()
	...
	elite = config.NeatConfig.Population * config.NeatConfig.ElitePopulation / 100 / #pool.species
	...
	for s = 1,#pool.species do
		...
		breed = config.NeatConfig.Population / #pool.species
		for i=1,breed do
			if i <= elite then table.insert(children, species.genomes[i]) 
			else table.insert(children, breedChild(species))
			end
